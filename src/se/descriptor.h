...
/* INFO
 * DACL defines permissions, e.g: read, write, execute for users or groups. It can grant or deny permissions to specific
 * userrs , groups or other security principals. If no DACL is present then full access is allowed. A DACL with explicit
 * deny ACEs can override any granted ACEs. SACL on the other hand is used to audit access attempts, read, writes, etc.
 * It's used by sysadmins.
 */
