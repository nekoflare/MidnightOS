...
#define APC_LEVEL                                                                                                      \
    1 // asynchronous procedure calls can be processed, execute functions in context of a thread for deferred work
...
KERNEL_API LPSTR StringifyIrql(KIRQL Irql);

KERNEL_API void KeRaiseIrql(KIRQL NewIrql, PKIRQL OldIrql);

KERNEL_API KIRQL KeGetCurrentIrql();

KERNEL_API void KeLowerIrql(KIRQL OldIrql);
